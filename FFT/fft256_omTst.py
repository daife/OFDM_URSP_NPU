...
if __name__ == "__main__":
    ...
    model_id, ret = acl.mdl.load_from_file("fft256.om")
    check_ret("acl.mdl.load_from_file fft256.om", ret)
    model_desc = acl.mdl.create_desc()
    ret = acl.mdl.get_desc(model_desc, model_id)
    check_ret("acl.mdl.get_desc fft256.om", ret)

    t = run_acl_model(model_id, model_desc, x_batch)
    print(f"fft256.om 32个batch总推理时间: {t*1000:.2f} ms")

    ret = acl.mdl.unload(model_id)
    check_ret("acl.mdl.unload fft256.om", ret)
    ret = acl.mdl.destroy_desc(model_desc)
    check_ret("acl.mdl.destroy_desc fft256.om", ret)
